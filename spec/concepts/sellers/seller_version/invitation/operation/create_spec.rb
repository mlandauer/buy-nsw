...
RSpec.describe Sellers::SellerVersion::Invitation::Create do
  ...
  it 'creates a new user' do
    result = described_class.call({ application_id: application.id, invitation: { email: 'blah@example.org' } }, 'config.current_user' => current_user)
  ...
  context 'with errors on the User model' do
    it 'sets them on the contract' do
      result = described_class.call({ application_id: application.id, invitation: { email: current_user.email } }, 'config.current_user' => current_user)
